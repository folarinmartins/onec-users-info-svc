...
class UserControllerTest extends TestCase{
	...
	public static function tearDownAfterClass(): void{
		if(self::$stub){
			...
			Utility::curl('http://localhost:8000/users/'.self::$stub,[],true,$curlOpt);
		}
	}
	function testCreate(){
		$data = array(
		...
		$response = Utility::curl('http://localhost:8000/users',[],true,$curlOpt);
	...
	function testGetAll(){
		$response = Utility::curl('http://localhost:8000/users',[]);
	...
	function testGetById(){
		if(self::$stub){
			...
			$response = Utility::curl('http://localhost:8000/users/'.self::$stub,[]);
			$this->assertArrayHasKey('data',$response);
			$this->assertNotEmpty($response['data']);
	...
	function testDelete(){
		if(self::$stub){
			...
			$response = Utility::curl('http://localhost:8000/users/'.self::$stub,[],true,$curlOpt);
