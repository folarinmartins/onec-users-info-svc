...
class Blockchain {
    ...
    public function __construct($api_code=null, $main_password=null,string $guid=null,string $second_password=null) {
        ...
        curl_setopt($this->ch, CURLOPT_CAINFO, dirname(__FILE__).'/Blockchain/ca-bundle.crt'); // /etc/letsencrypt/live
        // curl_setopt($this->ch, CURLOPT_CAINFO, '/etc/letsencrypt/live/blockstale.com/fullchain.pem'); // /etc/letsencrypt/live
        // curl_setopt($this->ch, CURLOPT_CAPATH, '/etc/letsencrypt/live/blockstale.com/'); // 

        $this->Create    = new Create($this);
        $this->Explorer  = new Explorer($this);
        $this->Push      = new Push($this);
        $this->Rates     = new Rates($this);
        $this->ReceiveV2 = new ReceiveV2($this->ch,$this->api_code);
        $this->Stats     = new Stats($this);
        $this->Wallet    = new Wallet($this,$main_password,$guid,$second_password);
    }
    ...
    private function _call() {
        ...
        if(self::DEBUG) {
			...
			// Utility::log('printing log at Blockchain:179...');
			// Utility::log(curl_getinfo($this->ch));
		}
        return $json;
    }
}
